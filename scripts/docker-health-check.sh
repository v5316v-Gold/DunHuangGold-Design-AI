#!/usr/bin/env bash
# ==========================================
# 敦煌金 AI 设计平台 - Docker 健康检查脚本
# ==========================================
# 用途：检测 WSL Docker Desktop 端口转发 ECONNREFUSED
#       自动恢复（重启 Docker Desktop 后等 30s 再验证）
#
# 用法：
#   bash scripts/docker-health-check.sh        # 检查一次
...
PYTHON_BIN="/c/Users/v5316/.hermes-web-ui/desktop-runtime/hermes/0.20.0/win-x64/node/npm/node"  # 任意 Node 二进制用于 TCP 测试（未使用，仅占位）
...
restart_docker() {
  log "⚠️  检测到 ECONNREFUSED，重启 Docker Desktop..."
