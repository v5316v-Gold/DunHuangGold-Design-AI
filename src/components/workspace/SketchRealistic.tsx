...
import { Upload, Download, RefreshCw, Sparkles, PenTool, X, ZoomIn, ZoomOut, RotateCw, Clock, Plus, Trash2, CheckSquare, Square } from 'lucide-react';
...
export default function SketchRealistic({ power, onDeductPower }: WorkspaceProps) {
  const [result, setResult] = useState<string | null>(null);
  const [mode, setMode] = useState<'sketch2real' | 'real2sketch'>('sketch2real');
  const [resolution, setResolution] = useState('2k');
  const [ratio, setRatio] = useState('auto');
  const [prompt, setPrompt] = usePageState('sketchRealistic-prompt', '');
  const [showHistory, setShowHistory] = useState(false);
  const [selectedHistory, setSelectedHistory] = useState<Set<string>>(new Set());
  const [zoom, setZoom] = useState(1);

  const cost = getTaskCost('sketch');
  ...
  const { handleDownload } = useDownload('sketch-realistic');
  ...
  const handleGenerate = async () => {
    if (!uploadedImage) { setError('请先上传线稿图'); return; }
    await generate({ image: uploadedImage, mode, resolution, ratio, prompt }, mode === 'sketch2real' ? '线稿转写实' : '写实转线稿');
  };
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-[340px] min-w-[340px] bg-[var(--bg-secondary)] border-r border-[var(--border-color)] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            <div className="p-5">
              <h2 className="text-sm font-semibold text-[var(--text-primary)] mb-5">线稿转写实</h2>

              {/* 参考图片 */}
              <div className="mb-5">
                <label className="param-label">参考图片</label>
                <div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} className={cn('border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer', isDragging ? 'border-[var(--gold)] bg-[var(--gold-muted)]' : 'border-[var(--border-color)] hover:border-[var(--gold)]')}>
                  {uploadedImage ? (
                    <div className="relative">
                      <Image src={uploadedImage} alt="已上传" width={400} height={300} className={cn('max-h-40 mx-auto rounded-lg', mode === 'sketch2real' ? 'grayscale' : '')} unoptimized />
                  ...
                  <input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" id="sketch-upload" />
                  <label htmlFor="sketch-upload" className="mt-3 inline-block px-4 py-2 text-sm bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg cursor-pointer hover:border-[var(--gold)]">选择图片</label>
                </div>
              ...
              <div className="mb-5">
                ...
                <div className="relative group">
                  <textarea
                    ...
                    placeholder={mode === 'sketch2real' ? '描述写实效果...' : '描述线稿风格...'}
                    className="w-full h-20 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 pr-16 text-sm resize-none focus:border-[var(--gold)] focus:outline-none"
              ...
              {/* 转换模式 */}
              <div className="mb-5">
                <label className="param-label">转换模式</label>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setMode('sketch2real')} disabled={isGenerating} className={cn('param-btn', mode === 'sketch2real' ? 'param-btn-selected' : 'param-btn-unselected')}>
                    <div className="font-medium text-sm">线稿转写实</div>
                  </button>
                  <button onClick={() => setMode('real2sketch')} disabled={isGenerating} className={cn('param-btn', mode === 'real2sketch' ? 'param-btn-selected' : 'param-btn-unselected')}>
                    <div className="font-medium text-sm">写实转线稿</div>
                  </button>
                </div>
              </div>
          ...
          <div className="p-5 border-t border-[var(--border-color)]">
            <button onClick={handleGenerate} disabled={isGenerating || !uploadedImage} className="w-full h-10 bg-gradient-to-r from-[var(--gold)] to-[var(--gold-hover)] text-black font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all hover:shadow-lg shadow-[0_0_12px_rgba(200,164,92,0.4)]">
              {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />转换中...</> : <><PenTool className="w-4 h-4" />开始转换 <span className="btn-power">⚡{cost}</span></>}
            </button>
        ...
        <div className="flex-1 bg-[var(--bg-primary)] flex items-center justify-center p-8 relative">
          {isGenerating ? (
            <div className="text-center">
              ...
              <p className="text-lg text-[var(--text-secondary)]">AI 转换中...</p>
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-2">
                ...
                <button onClick={() => handleDownload(result, 'png')} className="w-8 h-8 rounded bg-[var(--bg-card)] border"><Download className="w-4 h-4" /></button>
              </div>
            </div>
          ) : (
            <div className="text-center">
              <div className="w-48 h-48 border-2 border-dashed border-[var(--border-color)] rounded-xl flex flex-col items-center justify-center mb-4 hover:border-[var(--gold)]">
                <PenTool className="w-10 h-10 text-[var(--text-muted)] mb-2" />
              </div>
              <p className="text-base text-[var(--text-primary)] mb-1">上传线稿开始转换</p>
              <p className="text-sm text-[var(--text-muted)]">AI 线稿转写实引擎就绪</p>
            </div>
          )}
        </div>
