...
import { Upload, Download, RefreshCw, Wand2, X, ZoomIn, ZoomOut, Clock, Trash2, CheckSquare, Square } from 'lucide-react';
...
export default function OneClickDesign({ power, onDeductPower }: WorkspaceProps) {
  const [uploadedImage, setUploadedImage] = useState<string | null>(null);
  const [prompt, setPrompt] = usePageState('oneClickDesign-prompt', '');
  ...
  const [designMode, setDesignMode] = useState('product');
  ...
  const cost = getTaskCost('oneclick');
  ...
  const { isGenerating, progress, error, generate, setError } = useAiGeneration({
    ...
    onSuccess: (data) => {
      ...
      if (imageList.length > 0 && imageList[0]) {
        ...
        addToHistory({ featureId: 'oneclick', imageUrl: imageList[0] });
      }
    },
  });
  ...
  const handleGenerate = async () => {
    ...
    await generate({ image: uploadedImage, ratio, resolution, prompt, mode: designMode }, '一键设计');
  };
  ...
  // 下载 Hook
  const { handleDownload } = useDownload('oneclick-design');

  const ratios = IMAGE_RATIOS;
  const resolutions = IMAGE_RESOLUTIONS;
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[340px] min-w-[340px] bg-[var(--bg-secondary)] border-r border-[var(--border-color)] flex flex-col">
          <div className="flex-1 overflow-y-auto scrollbar-hide p-5">
            <h2 className="text-sm font-semibold text-[var(--text-primary)] mb-5">一键设计</h2>
            ...
            <div className="mb-5">
              ...
              <div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} className={cn('border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer', isDragging ? 'border-[var(--gold)] bg-[var(--gold-muted)]' : 'border-[var(--border-color)] hover:border-[var(--gold)]')}>
                ...
                <input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" id="oneclick-upload" />
                <label htmlFor="oneclick-upload" className="mt-3 inline-block px-4 py-2 text-sm bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg cursor-pointer hover:border-[var(--gold)]">选择图片</label>
              </div>
            ...
            <div className="mb-5">
              ...
              <PromptInput
                ...
                placeholder="描述设计要求，如：现代简约风格..."
              />
            </div>

                        {/* 模式选择 */}
            <div className="mb-5">
              <label className="param-label">模式（可选）</label>
              <div className="grid grid-cols-2 gap-2">
                {MODES.map((m) => (
                  <button
                    key={m.value}
                    onClick={() => setDesignMode(m.value)}
                    disabled={isGenerating}
                    className={cn(
                      'param-btn',
                      designMode === m.value ? 'param-btn-selected' : 'param-btn-unselected'
                    )}
                  >
                    {m.label}
                  </button>
          ...
          <div className="p-5 border-t border-[var(--border-color)]">
            <button onClick={handleGenerate} disabled={isGenerating || !uploadedImage} className="w-full h-10 bg-gradient-to-r from-[var(--gold)] to-[var(--gold-hover)] text-black font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all hover:shadow-lg shadow-[0_0_12px_rgba(200,164,92,0.4)]">
              {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />设计中...</> : <><Wand2 className="w-4 h-4" />开始设计 <span className="btn-power">⚡{cost}</span></>}
            </button>
        ...
        <div className="flex-1 bg-[var(--bg-primary)] flex items-center justify-center p-8 relative">
          {isGenerating ? (
            <div className="text-center">
              <div className="w-32 h-32 bg-[var(--bg-card)] rounded-2xl flex items-center justify-center mx-auto mb-6 border relative">
              ...
              <p className="text-lg text-[var(--text-secondary)] mb-2">AI 一键设计中...</p>
              <p className="text-sm text-[var(--text-muted)]">{Math.round(progress)}%</p>
            </div>
          ) : result ? (
            <div className="relative">
              <Image src={result} alt="设计结果" width={800} height={600} style={{ transform: `scale(${zoom})` }} className="max-w-full max-h-[65vh] object-contain rounded-xl shadow-2xl" unoptimized />
            ...
            <div className="text-center">
              <div className="w-48 h-48 border-2 border-dashed border-[var(--border-color)] rounded-xl flex flex-col items-center justify-center mb-4 hover:border-[var(--gold)]">
                <Wand2 className="w-10 h-10 text-[var(--text-muted)] mb-2" />
              </div>
              <p className="text-base text-[var(--text-primary)] mb-1">上传产品图片开始一键设计</p>
              <p className="text-sm text-[var(--text-muted)]">AI 智能设计引擎就绪</p>
            </div>
          )}
        </div>
