...
import { Upload, Download, RefreshCw, Eraser, X, ZoomIn, ZoomOut, Clock, Trash2, CheckSquare, Square } from 'lucide-react';
...
import { IMAGE_RESOLUTIONS, IMAGE_RATIOS, WorkspaceProps } from '@/constants/workspace';

export default function RemoveBackground({ power, onDeductPower }: WorkspaceProps) {
  ...
  const cost = getTaskCost('removebg');
  ...
  const handleGenerate = async () => {
    ...
    await generate({ image: uploadedImage, resolution: resolution, ratio }, '移除背景');
  };
  ...
  useKeyboardShortcuts({
    shortcuts: [
      ...
      { key: 'Escape', handler: () => { setError(null); }, description: '清空错误' },
    ],
    ignoreInput: false,
  });
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* 左侧设置面板 */}
        <div className="w-[320px] bg-[var(--bg-secondary)] border-r flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto scrollbar-hide">
              <div className="p-5">
                <h2 className="text-sm font-semibold text-[var(--text-primary)] mb-5">移除背景</h2>

            {/* 图片上传 */}
            <div className="mb-5">
              <div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} className={cn('border-2 border-dashed rounded-xl p-6 text-center', isDragging ? 'border-[var(--gold)] bg-[var(--gold-muted)]' : 'hover:border-[var(--gold)]')}>
                {uploadedImage ? (
                  <div className="relative">
                    <Image src={uploadedImage} alt="已上传" width={400} height={300} className="max-h-40 mx-auto rounded" unoptimized />
                ...
                <input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" id="bg-remove-upload" />
                <label htmlFor="bg-remove-upload" className="mt-3 inline-block px-4 py-2 text-sm bg-[var(--bg-card)] border rounded-lg cursor-pointer">选择图片</label>
              </div>
          ...
          <div className="p-5 border-t border-[var(--border-color)]">
            <button onClick={handleGenerate} disabled={isGenerating || !uploadedImage} className="w-full h-10 bg-gradient-to-r from-[var(--gold)] to-[var(--gold-hover)] text-black font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all hover:shadow-lg shadow-[0_0_12px_rgba(200,164,92,0.4)]">
              {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />处理中...</> : <><Eraser className="w-4 h-4" />移除背景 <span className="btn-power">⚡{cost}</span></>}
            </button>
        ...
        <div className="flex-1 bg-[var(--bg-primary)] flex items-center justify-center p-8 relative">
          {isGenerating ? (
            ...
            <div className="text-center text-[var(--text-muted)]">上传图片后开始移除背景</div>
          )}
        </div>
